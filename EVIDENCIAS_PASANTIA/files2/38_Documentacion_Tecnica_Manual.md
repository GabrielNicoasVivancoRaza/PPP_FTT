...
| Descripción del modelo `User` con campos `primerAcceso` y `puntoTrabajo` | ✅ Coincide con `User.js` | Sin cambios |
...
| DT-07 | Existen dos colecciones de MongoDB con los mismos tickets (`FechaUno` y `Lumineers_Canje`). `ticketController.js` opera sobre `Lumineers_Canje` (vía `req.TicketModel`), mientras que `puntoVentaController.js` importa el modelo `Ticket` fijo apuntado a `FechaUno`. Los canjes se escriben en una colección y se leen desde la otra, haciendo que el estado de canje sea invisible en el flujo normal de Staff y Jefe | Los canjes quedan registrados en Auditoría pero la interfaz los muestra como no canjeados; los contadores del dashboard no reflejan el estado real | Alta (afecta la operación en producción) |
| DT-08 | El enrutamiento de eventos Socket.IO en `canjeTicket` y `bulkCanjeTickets` emite a `punto-venta-${ticket.puntoVenta}` y `staff-${req.user.puntoTrabajo}`, pero `ticket.puntoVenta` no existe como campo en el schema de `Ticket` (el campo real es `Seat`/localidad), y `req.user.puntoTrabajo` es `undefined` para el rol Jefe. Las salas de WebSocket resultan siempre `undefined`, por lo que ningún cliente recibe la actualización en tiempo real al realizarse un canje | Los canjes no se propagan en tiempo real entre puntos de venta ni entre pestañas del mismo usuario | Media (la funcionalidad existe pero no opera en producción) |

## 7. Conclusiones del día

La documentación técnica del proyecto queda consolidada y verificada contra el estado real del código, con tres documentos actualizados (`README.md`, `SETUP.md`, `ARQUITECTURA.md`) para eliminar referencias a funcionalidades ya corregidas. Se incorporó explícitamente la aclaración sobre el flujo correcto de creación del usuario administrador solicitada por el tutor empresarial, y se completó el manual técnico de cierre con el nuevo script de verificación del Día 37. Se deja un registro priorizado de 8 items de deuda técnica para guiar futuras iteraciones, incluyendo dos hallazgos de alto impacto en producción: la divergencia de colecciones de MongoDB que hace invisible el estado de canje en la interfaz, y el enrutamiento incorrecto de Socket.IO que impide la propagación en tiempo real de los canjes.

**Observaciones:** Tres documentos técnicos actualizados tras verificación vs. código; 8 items de deuda técnica registrados (2 de ellos con impacto directo en producción); manual de cierre consolidado.
