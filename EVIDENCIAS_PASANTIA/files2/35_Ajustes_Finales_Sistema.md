...
| Pendiente de la validación | Acción tomada |
|---|---|
| Preparar guía visual simple para el rol Staff | Programado como entregable formal (Capacitación básica a usuarios), borrador v0.1 iniciado |
| Documentar claramente las credenciales iniciales del sistema | Programado como parte del manual técnico, incorporando el hallazgo de la Semana 6 sobre el flujo correcto de creación de administrador (`setup.js` como única fuente) |
...
| Sin valores hardcodeados de eventos anteriores en frontend  | ✅ Se mantiene |
| Arranque de backend y frontend sin errores | ✅ Se mantiene |
...
| RF-02 | Cambio de contraseña obligatorio en primer acceso | `primerAcceso: true` en el usuario fuerza redirección a pantalla de cambio antes de acceder a cualquier ruta | Semana 4 |
...
| RF-04 | Control de acceso por roles (Jefe / Staff / Impresor) | `ProtectedRoute` en frontend + middleware `authorize()` en backend verificados (casos PR-01 a PR-06) | Semana 4 |
...
| RF-10 | Gestión de Puntos de Venta y sus localidades | CRUD completo de puntos de venta y asignación de localidades verificado en validación | Semana 6 |
...
|---|---|---|
| RNF-01 | Tiempo de respuesta de endpoints < 500 ms bajo carga normal | Baseline medido: promedio de 7–10 ms en local; se espera < 300 ms en producción con Render activo |
| RNF-02 | Autenticación basada en JWT con expiración configurada | Token firmado con `JWT_SECRET`; expiración verificada en caso PB-04: token expirado devuelve 401 |
| RNF-03 | Rate limiting global de peticiones por IP | Caso PB-01 : ráfaga de más de 200 peticiones/minuto desde la misma IP devuelve 429; el límite aplica globalmente a todas las rutas, no solo a las de autenticación |
| RNF-04 | Encabezados de seguridad HTTP (Helmet) | Verificados al arrancar el servidor en; respuesta de `/health` incluye `X-Content-Type-Options`, `X-Frame-Options`, etc. |
| RNF-05 | CORS restringido al dominio del frontend | Verificación explícita realizada en (Sección 9): orígenes no autorizados no reciben `Access-Control-Allow-Origin` |
| RNF-06 | Logging estructurado de eventos del sistema | Logs verificados en (arranque) y (nombre de servicio corregido); formato JSON con campos `level`, `message`, `service`, `timestamp` |
| RNF-07 | Actualizaciones en tiempo real entre puntos de venta vía WebSocket | Socket.IO verificado en y en ronda de pruebas (múltiples clientes simultáneos) |
...
| RNF-09 | Índices de base de datos para campos de búsqueda frecuente | Índices verificados en log de arranque: `Transaction ID`, `usuario`, `nombre` de PuntoVenta |
...
|---|---|---|---|
| DT-01 | Auditoría potencialmente duplicada en rutas de canje y reimpresión (middleware + controlador) | Revisión extendida | Media |
...
| DT-03 | Credenciales de MongoDB expuestas en historial de Git de `render.yaml`; requiere rotación en Atlas | pendiente activo | Alta |
