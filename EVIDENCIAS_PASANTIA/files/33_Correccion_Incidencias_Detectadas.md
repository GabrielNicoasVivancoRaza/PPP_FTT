...
- Contraseñas hardcodeadas → La contraseña inicial se lee de `process.env.DEFAULT_PASSWORD`. Si la variable no está definida, el script usa `'sistema-inicial'` como fallback. Este fallback es aceptable para el contexto (es el valor documentado como credencial inicial), no representa un riesgo de seguridad equivalente al del fallback de localidades corregido en la Semana 5.
...
Todas las incidencias funcionales y de seguridad detectadas durante el desarrollo (5 en total) quedan corregidas y verificadas, con casos de regresión manual documentados para cada una. La revisión de seguridad extendida sobre archivos no modificados previamente no encontró hallazgos críticos nuevos, y la búsqueda de valores hardcodeados en el frontend confirmó que no existen referencias al proyecto anterior ni URLs de desarrollo en el código fuente. Se documentan dos observaciones abiertas para iteraciones futuras: la rotación de credenciales en `render.yaml` (ya conocida) y la arquitectura de auditoría duplicada (nueva, sin impacto funcional inmediato).

**Observaciones:** Pendiente para la Semana 8: rotar credenciales expuestas en `render.yaml`. Observación nueva: auditoría potencialmente duplicada en rutas de tickets, sin impacto funcional inmediato.
